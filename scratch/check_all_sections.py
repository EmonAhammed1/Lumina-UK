...
for f in sorted(glob.glob('sections/*.liquid')):
    with open(f, 'r', encoding='utf-8', errors='ignore') as fp:
        ...
        for idx, line in enumerate(lines):
            ...
            clean_l = re.sub(r'filter\.[^\"]+', '', clean_l)
            clean_l = re.sub(r'\.descriere-[^\s]+', '', clean_l)
            clean_l = re.sub(r'\b(return|return_to|returns)\b', '', clean_l, flags=re.IGNORECASE)
            clean_l = re.sub(r'\b(nanofibre|nanofibres|nanofiber|nanofibers)\b', '', clean_l, flags=re.IGNORECASE)
            clean_l = re.sub(r'\b(technology)\b', '', clean_l, flags=re.IGNORECASE)
            clean_l = re.sub(r'\{\%-?\s*comment\s*-?\%\}[\s\S]*?\{\%-?\s*endcomment\s*-?\%\}', '', clean_l)
            clean_l = re.sub(r'//[^\n]+', '', clean_l)
